...
- 生成页：已进入可验收版雏形，支持常见增删改查式自然语言指令，但暂不做复杂多轮工具规划。

当前明确边界：

- 不调用大模型即可回答的问题，优先使用规则统计。
- 生成页已有草稿后的增量修订优先使用确定性规则，模糊指令不交给模型自由发挥，避免用户草稿被随机改写。
- 数据页不把全量提交记录发送给大模型。
...
- 初次生成。
- 基于 existingSchema 增量修改，包括新增常见字段、删除字段、修改字段类型、修改必填规则、修改字段文案 / 占位提示、修改选择题选项。
- 只读检查，例如检查字段数量、移动端填写长度和字段引导，不擅自改动草稿。
- Schema 校验。
- 输出变更摘要。
- 右侧预览实时同步。
...
- 用户输入“把姓名字段文案改成怎么称呼你？”，右侧能更新字段文案。
- 用户输入“把满意度选项改成非常满意、满意、一般、不满意”，右侧能更新选项。
...
- 用户输入“找出没有上传发票的记录”，Agent 能返回匹配提交和结构化 filter，并联动右侧列表过滤。

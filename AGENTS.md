...
## Cold-Start Growth & Session Audit Rules (微量流量冷启动行为审计规则)

在网站处于冷启动阶段（日均 UV < 100），AI 团队在进行 SEO/增长分析和产品迭代判定时，应遵守以下规则：
- **禁止单一依赖统计大盘**：在 `Observe` 向 `Decide` 状态流转时，严禁仅依靠 GA4 漏斗百分比进行决策。必须通过 Microsoft Clarity 等工具走查过去 7 天的真实 Session 录屏（通常为 10-20 个），通过真实行为轨迹发现产品硬伤。
- **重点盯防三大核心卡点**：
  1. **阅读与信任流失**：审计用户在落地博客/解决方案页的停留时长与滚动深度，判断引言与排版是否有说服力。
  2. **转化意图中断**：审计用户对主 CTA 按钮的 Hover 与点击反应，排查非按钮区域的 Rage Clicks。
  3. **Demo 流程受挫**：审计用户在 `/forms/new` 输入框前的停顿时间，判断是否需要 Prompt 示例，并检查 AI 生成时的等待焦虑状态。
- **冷冻期热修复例外（Hotfix Exception）**：在 `Observe` 冷冻期内，若发现影响 CLS（布局偏移）崩溃、404 死链或核心 CTA 失效等严重阻碍体验的物理 Bug，必须立刻执行 Hotfix 热修复，不受观察期不改动的教条限制。

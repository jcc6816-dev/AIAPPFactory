  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        await sleep(180);
        send({ type: "thinking", message: "正在规划字段结构、填写顺序和单题流体验..." });
        await sleep(180);
        send({
          type: "tool_start",
          tool: "generate_form_schema",
        ...
        send({
          type: "thinking",
          message: `已生成 ${generated.schema.fields.length} 个字段，正在同步右侧预览沙盒...`,
        });
        await sleep(120);
        send({
          type: "draft_updated",
        ...
        send({ type: "done", message: "生成完成，你可以继续用自然语言要求我微调。" });
        controller.close();
      } catch (error: any) {
